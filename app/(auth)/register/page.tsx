                ...
                <form className="space-y-5" onSubmit={handleRegister}>
                  <div>
                    ...
                    <div className="mt-1 relative">
                      ...
                      <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10 block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" placeholder="you@company.com" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-1 relative">
                      ...
                      <input type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10 pr-10 block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" placeholder="••••••••" />
                  ...
                  <div>
                    ...
                    <div className="mt-1 relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" />
                      <input type={showPassword ? "text" : "password"} required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-10 block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" placeholder="••••••••" />
                    </div>
                  </div>
            ...
            {step === 'OTP' && (
              <motion.form key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6" onSubmit={handleVerifyOTP}>
                <div className="text-center">
                  ...
                  <input type="text" required value={otp} onChange={(e) => setOtp(e.target.value)} maxLength={6} className="block w-full px-4 py-4 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 text-center tracking-[0.75em] font-bold text-2xl text-slate-900" placeholder="------" />
                </div>
                <button type="submit" disabled={loading || otp.length < 6} className="w-full flex justify-center py-3 px-4 rounded-xl text-sm font-semibold text-white bg-sky-600 hover:bg-sky-700">
                ...
                <div className="text-center pt-2">
                  {countdown > 0 ? (
                    ...
                    <button type="button" onClick={handleResendOTP} disabled={resendLoading} className="text-sm font-medium text-sky-600 flex items-center justify-center gap-2 mx-auto">
              ...
              <motion.form key="onboarding" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-5" onSubmit={handleOnboarding}>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" required value={metadata.firstName} onChange={(e) => setMetadata({...metadata, firstName: e.target.value})} className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Last Name</label>
                    <input type="text" required value={metadata.lastName} onChange={(e) => setMetadata({...metadata, lastName: e.target.value})} className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Mobile Number</label>
                  <input type="tel" required value={metadata.mobileNumber} onChange={(e) => setMetadata({...metadata, mobileNumber: e.target.value})} className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Date of Birth</label>
                    <input type="date" required value={metadata.dob} onChange={(e) => setMetadata({...metadata, dob: e.target.value})} className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Gender</label>
                    <select required value={metadata.gender} onChange={(e) => setMetadata({...metadata, gender: e.target.value})} className="block w-full px-4 py-3 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500/20 sm:text-sm">
