        ...
        <div className="w-full lg:w-1/3 bg-white rounded-3xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50">
            <select 
              ...
              className="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none font-medium text-slate-700 text-sm shadow-sm"
            >
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-1.5 text-xs font-medium text-slate-500 truncate">
                      {ticket.creatorRole === 'DEALER' ? <Shield className="w-3 h-3 text-sky-500" /> : <User className="w-3 h-3" />}
          ...
          {activeTicket ? (
              ...
              <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    <span className="font-medium text-slate-500 flex items-center gap-1.5">
                      {activeTicket.creatorRole === 'DEALER' ? <Shield className="w-4 h-4 text-sky-500" /> : <User className="w-4 h-4 text-slate-400" />}
                ...
                <div className="flex flex-shrink-0">
                  <select 
                    ...
                    className={`text-sm font-bold px-4 py-2.5 rounded-xl border outline-none cursor-pointer shadow-sm ${
                      activeTicket.status === 'OPEN' ? 'bg-amber-50 text-amber-700 border-amber-200 focus:ring-2 focus:ring-amber-500' :
                    ...
                    return (
                      <div key={idx} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex items-end gap-2 max-w-[85%] ${isAdmin ? 'flex-row-reverse' : 'flex-row'}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${isAdmin ? 'bg-sky-500 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>
                          ...
                          <div className={`p-4 rounded-2xl text-sm shadow-sm ${
                            ...
                            <p className={`text-[10px] mt-2 font-medium ${isAdmin ? 'text-sky-100' : 'text-slate-400'}`}>
                ...
                <div className="p-4 bg-white border-t border-slate-100">
                  <form onSubmit={handleSendMessage} className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:bg-white transition-all"
                    />
                    <button 
                      type="submit"
                      disabled={!newMessage.trim()}
                      className="w-12 h-12 bg-sky-500 text-white rounded-full flex items-center justify-center hover:bg-sky-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md shadow-sky-500/20 flex-shrink-0"
                    >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center bg-slate-50/50">
              <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-sm border border-slate-100 mb-6">
                <LifeBuoy className="w-10 h-10 text-sky-200" />
              </div>
