                ...
                {PINNED_CONTACTS.map(contact => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-2xl border transition-all ${activeContact?.contactId === contact.contactId
                        ? 'bg-slate-900 border-slate-900 shadow-md ring-2 ring-sky-500/50'
        ...
        <div className="w-full lg:w-2/3 flex flex-col min-h-0 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          {activeContact ? (
                        ...
                        <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`flex items-end gap-2 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            <div className={`p-4 rounded-2xl text-sm shadow-sm flex flex-col ${msg.isSystemRecord ? 'bg-slate-800 text-white rounded-br-sm' :
                                isMe ? 'bg-sky-500 text-white rounded-br-sm' :
                                  'bg-white border border-slate-100 text-slate-700 rounded-bl-sm'
                              }`}>
                              <p className="whitespace-pre-wrap leading-relaxed break-words">{msg.content}</p>
                              <div className={`flex items-center gap-1.5 mt-2 justify-end`}>
                                <p className={`text-[10px] font-medium ${isMe || msg.isSystemRecord ? 'text-sky-100 opacity-80' : 'text-slate-400'}`}>
                                  {new Date(msg.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </p>
                                {(isMe || msg.isSystemRecord) && (
                                  <span className={`text-[10px] uppercase font-black tracking-tighter ${msg.isSystemRecord ? 'text-emerald-400' : 'text-sky-200'}`}>
                                    • {msg.deliveryStatus}
                                  </span>
                                )}
                              </div>
              ...
              <div className="flex-shrink-0 p-4 bg-white border-t border-slate-100 z-10">
                <form onSubmit={handleSendMessage} className="flex gap-3 bg-slate-50 p-2 rounded-full border border-slate-200 focus-within:ring-2 focus-within:ring-sky-500/20 focus-within:bg-white transition-all">
                  ...
                  <button
                    ...
                    className={`w-10 h-10 text-white rounded-full flex items-center justify-center disabled:opacity-50 transition-all shadow-md flex-shrink-0 ${activeContact.role === 'SYSTEM_BULK' ? 'bg-slate-900 hover:bg-slate-800 shadow-slate-900/20' : 'bg-sky-500 hover:bg-sky-600 shadow-sky-500/20'
                      }`}
                  >
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 p-12 text-center h-full">
              ...
              <p className="text-xl font-bold text-slate-700 mb-2">Bharat Forge Chat Console</p>
