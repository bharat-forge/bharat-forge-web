          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 flex-shrink-0">
                <div>
                  <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
                    <UserCircle className="w-6 h-6 text-sky-500" /> Customer Profile
                  </h2>
                </div>
              ...
              <div className="p-6 overflow-y-auto flex-1 space-y-8">
                {detailsLoading ? (
                  <div className="flex justify-center p-12"><div className="w-8 h-8 border-4 border-slate-200 border-t-sky-500 rounded-full animate-spin"></div></div>
                ) : (
                  <>
                    ...
                    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 flex items-center gap-6">
                      <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center border-4 border-sky-100 shadow-sm">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          <span className={`px-2.5 py-1 rounded text-xs font-bold ${
                            userDetails.user.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                          }`}>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      ...
                      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <p className="text-3xl font-black text-sky-600">₹{(userDetails.stats.totalSpent || 0).toLocaleString()}</p>
                      </div>
                    </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden">
              ...
              <form onSubmit={handleSendNotification} className="p-6 space-y-5">
                ...
                <div>
                  ...
                  <input required type="text" value={notifyFormData.title} onChange={e => setNotifyFormData({...notifyFormData, title: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none" placeholder="e.g., Important Account Update" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">Message Body</label>
                  <textarea required rows={4} value={notifyFormData.message} onChange={e => setNotifyFormData({...notifyFormData, message: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-sky-500 outline-none resize-none" placeholder="Type your message here..." />
                </div>
                <div className="pt-4 flex gap-3">
