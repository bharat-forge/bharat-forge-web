...
import { 
  Lock, FileText, Plus, Search, 
  Trash2, CheckCircle2, AlertCircle, Edit, 
  ChevronRight, ChevronLeft, Eye, Clock
} from 'lucide-react';
import { getPaginatedPolicies, createPolicy, updatePolicy, activatePolicy, deletePolicy } from '@/api/admin/privacy';
...
export default function AdminPrivacyPage() {
  ...
  const [currentPolicy, setCurrentPolicy] = useState<Partial<Policy>>({ version: '', title: '', content: '' });
  const [activePreviewPolicy, setActivePreviewPolicy] = useState<Policy | null>(null);
  ...
  const fetchPolicies = async () => {
    setIsLoading(true);
    try {
      const res = await getPaginatedPolicies({ page, limit: 10, search });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (currentPolicy.id) {
        await updatePolicy(currentPolicy.id, currentPolicy);
      } else {
        await createPolicy(currentPolicy);
      }
      setIsModalOpen(false);
      setCurrentPolicy({ version: '', title: '', content: '' });
      fetchPolicies();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      alert('Cannot delete active policy');
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium">Manage data protection document versions</p>
          </div>
        </div>
        <button
          onClick={() => {
      ...
      <AnimatePresence>
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-3xl">
                <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                  ...
                  {currentPolicy.id ? 'Edit Version' : 'New Privacy Policy Version'}
                </h2>
                <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        placeholder="e.g. Global Privacy Policy"
        ...
        {isPreviewOpen && activePreviewPolicy && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">{activePreviewPolicy.title}</h2>
                  <p className="text-sm font-mono text-slate-500 mt-1">Version: {activePreviewPolicy.version}</p>
                </div>
                <button
