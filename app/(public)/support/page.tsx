        ...
        <div className="w-full lg:w-1/3 flex flex-col min-h-0 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="flex-shrink-0 p-6 border-b border-slate-100 flex items-center justify-between bg-slate-900 text-white">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <LifeBuoy className="w-5 h-5 text-sky-400" /> Support Desk
            </h2>
            <button 
          ...
          {activeTicket ? (
            <>
              ...
              <div ref={chatContainerRef} className="flex-1 min-h-0 overflow-y-auto p-6 bg-slate-50 space-y-6">
                {isChatLoading ? (
                  <div className="flex justify-center"><div className="w-8 h-8 border-2 border-slate-300 border-t-sky-500 rounded-full animate-spin"></div></div>
                ) : (
                    ...
                    return (
                      <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`flex items-end gap-2 max-w-[80%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${isMe ? 'bg-sky-100 text-sky-600' : 'bg-slate-200 text-slate-600'}`}>
                          ...
                          <div className={`p-4 rounded-2xl text-sm ${
                            ...
                            <p className={`text-[10px] mt-2 font-medium ${isMe ? 'text-sky-100' : 'text-slate-400'}`}>
                ...
                <div className="flex-shrink-0 p-4 bg-white border-t border-slate-100">
                  <form onSubmit={handleSendMessage} className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-sky-500"
                    />
                    <button 
                      type="submit"
                      disabled={!newMessage.trim()}
                      className="w-12 h-12 bg-sky-500 text-white rounded-full flex items-center justify-center hover:bg-sky-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
                    >
