  ...
  if (!term) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Terms Not Available</h1>
        <p className="text-slate-500">Legal documents are currently being updated.</p>
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center justify-center gap-6 text-sm font-medium text-slate-300">
                <span className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg border border-white/10">
                  <ShieldCheck className="w-4 h-4 text-sky-400" /> Version {term.version}
                </span>
                <span className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg border border-white/10">
                  <Clock className="w-4 h-4 text-sky-400" /> Last Updated: {new Date(term.updatedAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                </span>
              </div>
            </div>
          </div>
