...
import { 
  IndianRupee, ShoppingCart, ShieldCheck, 
  LifeBuoy, Activity, PieChart as PieChartIcon, TrendingUp, BarChart3, Calculator
} from 'lucide-react';
import { getDealerDashboardStats } from '@/api/dealer/dashboard';
import { 
...
export default function DealerDashboardPage() {
  ...
  const fetchStats = async () => {
    try {
      const res = await getDealerDashboardStats();
      setStats(res.data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <div className="p-6 lg:p-10 w-full space-y-6">
      ...
      <div className="flex items-center justify-between bg-white p-6 rounded-3xl border border-slate-200 shadow-sm w-full">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Dealer Portal Analytics</h1>
          <p className="text-slate-500 font-medium mt-1">Monitor your procurement and account health</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-6 w-full">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 font-bold text-xs mb-1 uppercase tracking-wider">Total Spent</p>
            <h3 className="text-2xl font-black text-slate-900">₹{(stats.metrics.totalSpent / 1000).toFixed(1)}k</h3>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 font-bold text-xs mb-1 uppercase tracking-wider">Total Orders</p>
            <h3 className="text-2xl font-black text-slate-900">{stats.metrics.totalOrders}</h3>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 font-bold text-xs mb-1 uppercase tracking-wider">Avg Order Value</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 font-bold text-xs mb-1 uppercase tracking-wider">Auth. Products</p>
            <h3 className="text-2xl font-black text-slate-900">{stats.metrics.authorizedProducts}</h3>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-500 font-bold text-xs mb-1 uppercase tracking-wider">Open Tickets</p>
        ...
        <div className="lg:col-span-8 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm w-full">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-sky-500" /> Spending Trend (6 Months)
            </h2>
          </div>
          <div className="w-full h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.charts.spendTrend} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                ...
                <Area type="monotone" dataKey="total" name="Spent" stroke="#0ea5e9" strokeWidth={4} fillOpacity={1} fill="url(#colorSpend)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm w-full">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-indigo-500" /> Spend by Category
            </h2>
          </div>
          <div className="w-full h-[350px]">
            {stats.charts.spendByCategory.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.charts.spendByCategory} layout="vertical" margin={{ top: 0, right: 30, left: 40, bottom: 0 }}>
                  ...
                  <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#475569', fontSize: 12, fontWeight: 700}} width={140} />
                  <Tooltip content={<CustomTooltip prefix="₹" />} />
                  <Bar dataKey="total" name="Spent" fill="#6366f1" radius={[0, 8, 8, 0]} barSize={32} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm w-full">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <Activity className="w-5 h-5 text-emerald-500" /> Monthly Order Volume
            </h2>
          </div>
          <div className="w-full h-[350px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={stats.charts.spendTrend} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12, fontWeight: 600}} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="volume" name="Orders Placed" stroke="#10b981" strokeWidth={4} dot={{ r: 6, fill: '#10b981', strokeWidth: 0 }} activeDot={{ r: 8, strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
